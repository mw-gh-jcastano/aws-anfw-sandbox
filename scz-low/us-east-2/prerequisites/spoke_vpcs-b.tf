...
#      Name = format("%s-%s-spoke-vpc-b", var.security_zone, var.region)
      Name = format("%s-%s-spoke-vpc-b", local.security_zone, local.region)
    },
  )
}
...
#      Name = format("%s-%s-spoke-vpc-b/${data.aws_availability_zones.available.names[count.index]}/protected-subnet", var.security_zone, var.region)
      Name = format("%s-%s-spoke-vpc-b/${data.aws_availability_zones.available.names[count.index]}/protected-subnet", local.security_zone, local.region)
    },
  )
}
...
#      Name = format("%s-%s-spoke-vpc-b/${data.aws_availability_zones.available.names[count.index]}/endpoint-subnet", var.security_zone, var.region)
      Name = format("%s-%s-spoke-vpc-b/${data.aws_availability_zones.available.names[count.index]}/endpoint-subnet", local.security_zone, local.region)
    },
  )
}
...
#      Name = format("%s-%s-spoke-vpc-b/route-table", var.security_zone, var.region)
      Name = format("%s-%s-spoke-vpc-b/route-table", local.security_zone, local.region)
    },
  )
}
